...
%% GRAFICO
figure; subplot (1,2,1); plot(matrice_w'), xlabel('ITER'), ylabel('w'); title(sprintf('Grafico convergenza della rete neurale'));
subplot(1,2,2); plot(conv), xlabel('ITER'), ylabel('y'); title(sprintf('Grafico convergenza y'));
figure; plot(1:dimensione_im_x*dimensione_im_y,y,'r',1:dimensione_im_x*dimensione_im_y,A(:),'b');
        legend('NN output','Original image')

%% DISEGNO COMPARATIVO
figure;
subplot(1,3,1);  imshow(A(:,:,1)); title(sprintf('Immagine originale'));
subplot(1,3,2);  imshow(B); title(sprintf('Immagine sfocata'));
subplot(1,3,3);  imshow(y_m); title(sprintf('Immagine ricostruita'));
